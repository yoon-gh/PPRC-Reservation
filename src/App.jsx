...
const USER_SECTION = Object.freeze({
  OVERVIEW: "overview",
  RESERVE: "reserve",
});
...
function getStatusLabel(status) {
  return (
    FACILITY_STATUS_LABEL[status] || RESERVATION_STATUS_LABEL[status] || status
  );
}
...
function filterReservationsByCategory(reservations, category) {
  ...
  return reservations.filter(
    (reservation) => reservation.category === category,
  );
}
...
function filterReservationsByMonth(reservations, month) {
  ...
  return reservations.filter((reservation) =>
    rangesOverlap(
      toDate(reservation.start),
      toDate(reservation.end),
      start,
      end,
    ),
  );
...
function getReservationStats(reservations) {
  return reservations.reduce(
    (acc, reservation) => {
      ...
      if (reservation.status === RESERVATION_STATUS.MAINTENANCE)
        acc.maintenance += 1;
...
function findReservationConflict(reservations, candidate, ignoreId = null) {
  ...
  return conflict
    ? {
        message: `중복 예약: ${conflict.title} (${formatDateTime(conflict.start)} ~ ${formatDateTime(conflict.end)})`,
      }
    : null;
}
...
function downloadReservationsCsv(
  reservations,
  filename = "pprc_reservations.csv",
) {
  const blob = new Blob(["\ufeff" + buildReservationsCsv(reservations)], {
...
function FacilityCard({ item }) {
  const showSensorMemo =
    item.name.includes("다중영상촬영실") && Array.isArray(item.sensors);
  return (
    <div className={`card facility ${item.status}`}>
      <div>
        ...
        <h3>{item.name}</h3>
        {showSensorMemo && (
          <div className="sensor-note">센서: {item.sensors.join(", ")}</div>
        )}
      </div>
  ...
  async function submitReservation(event) {
    ...
    const candidate = {
      ...
      status: isMaintenance
        ? RESERVATION_STATUS.MAINTENANCE
    ...
    setMessage({
      type: "success",
      text: "예약 신청이 저장되었습니다. 관리자가 승인하면 확정됩니다.",
    });
  }
    ...
    <div className="card form-card">
      <form onSubmit={submitReservation}>
        <div
          ...
          style={{ padding: 0, border: 0, marginBottom: 14 }}
        >
          <div>
            <h3>예약 신청</h3>
        ...
        {disabled && (
          <div className="setup-note">
            Supabase 환경변수가 설정되지 않아 저장 기능이 비활성화되었습니다.
          </div>
        )}
        <div className="form-grid">
          ...
          {!isMaintenance && (
            <label className="full">
              ...
              <input
                value={form.crop}
                onChange={(event) => updateForm("crop", event.target.value)}
                placeholder="예: 배추, 딸기, 고추"
              />
            </label>
          )}
          <label>
          ...
          {isImaging && (
            <label className="full">
              촬영 방식
              <select
                value={form.imagingMode}
                onChange={(event) =>
                  updateForm("imagingMode", event.target.value)
                }
              >
                <option>독립 촬영</option>
...
function MonthlyCalendar({
  ...
  const visibleReservations = useMemo(
    () => filterReservationsByCategory(reservations, selectedCategory),
    [reservations, selectedCategory],
  );
  const mobileAgenda = useMemo(
    () => groupReservationsByDay(visibleReservations, month),
    [visibleReservations, month],
  );
    ...
    <div className="card calendar-card">
      <div className="calendar-head">
        ...
        <div className="calendar-controls">
          <Button type="button" variant="light" onClick={() => moveMonth(-1)}>
            이전
          </Button>
      ...
      <div className="calendar-scroll">
        <div className="calendar-grid">
          ...
          {days.map(({ date, key, inMonth }) => {
            const dayReservations = getReservationsForDay(
              visibleReservations,
              date,
            );
              ...
              <div key={key} className={`day ${inMonth ? "" : "muted"}`}>
                ...
                {visible.map((reservation) => (
                  <div
                    ...
                    title={`${reservation.facility} / ${reservation.title}`}
                  >
                    {categoryIcon[reservation.category]} {reservation.facility}{" "}
                    · {reservation.title}
                  </div>
                ))}
                {extra > 0 && (
                  <div className="sensor-note">+{extra}건 더 있음</div>
                )}
              </div>
            );
          })}
            ...
            <div key={key} className="mobile-day-group">
              ...
              {items.map((reservation) => (
                ...
                >
                  ...
                  <div className="mobile-card-line">
                    {formatPeriod(reservation)} / {formatTime(reservation)}
                  </div>
                  <div className="mobile-card-line">
                    신청자: {reservation.user || "-"}
                  </div>
                </div>
              ))}
            </div>
...
function AdminLogin({ session, onLogin, onLogout, isAdmin }) {
  ...
  async function submit(event) {
    ...
    if (!isSupabaseConfigured || !supabase) {
      setMessage({
        type: "error",
        text: "Supabase 환경변수가 설정되지 않았습니다.",
      });
      return;
    }
  ...
  return (
    <div className="card login-card">
      ...
      {!isSupabaseConfigured && (
        <div className="setup-note">
          .env.local에 Supabase URL과 Anon Key를 설정해야 합니다.
        </div>
      )}
      {adminEmails.length === 0 && (
        <div className="setup-note">
          VITE_ADMIN_EMAILS에 관리자 이메일을 입력해야 합니다.
        </div>
      )}
      {message && (
        <div className={`message ${message.type}`}>{message.text}</div>
      ...
      >
        ...
        <Button
          type="submit"
          disabled={!isSupabaseConfigured || adminEmails.length === 0}
        >
          로그인
        </Button>
      </form>
...
function AdminReservationPanel({
  reservations,
    ...
    <div className="card table-card">
      <div className="table-head">
        <div>
          <h3>관리자 예약 승인·수정</h3>
          <p>
            전체 예약을 수정할 수 있으며, 다운로드는 현재 캘린더 월에 해당하는
            예약만 내려받습니다.
          </p>
        </div>
        <Button
          ...
          onClick={() =>
            downloadReservationsCsv(
              selectedMonthReservations,
              "pprc_reservations_current_month.csv",
            )
          }
        >
          엑셀용 CSV 다운로드
      ...
      {message && (
        <div className={`message ${message.type}`} style={{ margin: 16 }}>
          {message.text}
        </div>
      )}
                    ...
                    {isEditing ? (
                      <select
                        ...
                        onChange={(event) => {
                          ...
                          const firstFacility =
                            getFacilitiesByCategory(nextCategory)[0]?.name ||
                            "";
                          setDraft((prev) => ({
                            ...prev,
                  ...
                  <td>
                    {isEditing ? (
                      <select
                        value={row.facility}
                        onChange={(event) =>
                          updateDraft("facility", event.target.value)
                        }
                      >
                  ...
                  <td>
                    {isEditing ? (
                      <input
                        value={row.crop}
                        onChange={(event) =>
                          updateDraft("crop", event.target.value)
                        }
                      />
                    ) : (
                      reservation.crop
                  ...
                  <td>
                    {isEditing ? (
                      <input
                        value={row.user}
                        onChange={(event) =>
                          updateDraft("user", event.target.value)
                        }
                      />
                    ) : (
                      reservation.user
...
export default function App() {
  ...
  async function addReservation(reservation) {
    ...
    const { data, error } = await supabase
      .from("reservations")
      .insert(mapReservationToDb(reservation))
      .select()
      .single();
  ...
  async function updateReservation(id, patch) {
    ...
    const current = reservationsState.find(
      (reservation) => reservation.id === id,
    );
    const nextReservation = { ...current, ...patch };
    const { data, error } = await supabase
      .from("reservations")
      .update(mapReservationToDb(nextReservation))
      .eq("id", id)
      .select()
      .single();
    if (error) return { error: error.message };
    ...
    setReservationsState((prev) =>
      prev.map((reservation) =>
        reservation.id === id ? mapDbToReservation(data) : reservation,
      ),
    );
    return { ok: true };
  }
  ...
  const selectedMonthReservations = useMemo(
    () => filterReservationsByMonth(reservationsState, calendarMonth),
    [reservationsState, calendarMonth],
  );
  const filteredReservations = useMemo(
    () => filterReservationsByCategory(selectedMonthReservations, tab),
    [selectedMonthReservations, tab],
  );
  const reservationStats = useMemo(
    () => getReservationStats(selectedMonthReservations),
    [selectedMonthReservations],
  );
  ...
  const dashboardCards = [
    {
      ...
      icon: categoryIcon[CATEGORY.ALL],
      helper: `${calendarMonth.getFullYear()}년 ${calendarMonth.getMonth() + 1}월 기준`,
    },
    {
      label: "사용 가능 시설",
      value: facilityStatusStats[FACILITY_STATUS.AVAILABLE] || 0,
      icon: "🟢",
      helper: "현재 예약 가능 상태",
    },
    {
      label: "사용 중 시설",
      value: facilityStatusStats[FACILITY_STATUS.IN_USE] || 0,
      icon: "🔵",
      helper: "현재 사용 중",
    },
    {
      label: "해당 월 점검 등록",
      value: reservationStats.maintenance,
      icon: "🛠",
      helper: "관리자가 등록한 점검 일정",
    },
  ];
    ...
    <div className="app">
      <div className="container">
        <header className="header">
          <div>
            <p>
              <strong>Phenotyping Facility Reservation System</strong>
            </p>
            <h1>표현체 연구시설 예약 시스템</h1>
            <p>
              재배시설은 장기 재배 예약으로, 촬영시설은 시간 단위 예약으로 분리
              관리합니다. 예약 데이터는 Supabase 공용 DB에 저장됩니다.
            </p>
          ...
          <div className="mode-tabs">
            <button
              type="button"
              onClick={() => setViewMode(VIEW_MODE.USER)}
              className={viewMode === VIEW_MODE.USER ? "active" : ""}
            >
              예약현황
            </button>
            <button
              type="button"
              onClick={() => setViewMode(VIEW_MODE.ADMIN)}
              className={viewMode === VIEW_MODE.ADMIN ? "active" : ""}
            >
              관리자
            </button>
          </div>
        ...
        {viewMode === VIEW_MODE.USER && (
          <div className="user-layout">
            <div className="mode-tabs user-tabs">
              <button
                type="button"
                onClick={() => setUserSection(USER_SECTION.OVERVIEW)}
                className={
                  userSection === USER_SECTION.OVERVIEW ? "active" : ""
                }
              >
                예약현황
              </button>
              <button
                type="button"
                onClick={() => setUserSection(USER_SECTION.RESERVE)}
                className={userSection === USER_SECTION.RESERVE ? "active" : ""}
              >
                예약하기
              </button>
            </div>
              ...
              <>
                <div className="facility-sections">
                  <section className="user-growth-section facility-section">
                    <SectionTitle
                      icon={categoryIcon[CATEGORY.GROWTH]}
                      title="재배시설"
                      subtitle="기간 단위로 예약하고, 필요 시 촬영 예약과 연결합니다."
                    />
                    <div className="grid-5" style={{ marginTop: 14 }}>
                      {growthFacilities.map((item) => (
                  ...
                  <section className="user-imaging-section facility-section">
                    <SectionTitle
                      icon={categoryIcon[CATEGORY.IMAGING]}
                      title="촬영시설 및 장비"
                      subtitle="시간 단위 예약, 연계 촬영과 독립 촬영을 구분합니다."
                    />
                    <div className="grid-5" style={{ marginTop: 14 }}>
                      {imagingFacilities.map((item) => (
                        <FacilityCard key={item.id} item={item} />
                      ))}
                    </div>
                  </section>
                  ...
                  <div className="filter-row">
                    {[
                      ...
                      <button
                        ...
                        type="button"
                        onClick={() => setTab(name)}
                        className={`chip ${tab === name ? "active" : ""}`}
                      >
              ...
              <>
                <section className="two-col user-reservation-section">
                  ...
                  <div className="card rules">
                    <h3>예약 운영 규칙</h3>
                    <p>
                      <strong>재배 예약</strong>은 작목, 재배기간, 처리조건,
                      식물체 수를 기준으로 승인합니다.
                    </p>
                    <p>
                      <strong>촬영 예약</strong>은 촬영시설, 센서, 촬영시간,
                      연계 재배 예약 여부를 기준으로 승인합니다.
                    </p>
